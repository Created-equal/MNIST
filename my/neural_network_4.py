...
def main():
        ...
        hidden_neurons = int(input('The number of hidden neurons : '))
        cost = Quadratic()
        while True:
                """
                ...
                """
                which = int(input("Cost function : "))
                if which == 1 or which == 2 :
                        if which == 2 :
                                cost = CrossEntropy()
                        break
        lmbda = float(input('The regularization parameter lambda used for L2-regularization : '))
